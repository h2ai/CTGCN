...
class DynGEMLoss(nn.Module):
    ...
    def __init__(self, alpha, beta, nu1, nu2, device):
...
class DynGEMBatchGenerator:
    ...
    def __init__(self, node_list, batch_size, beta, shuffle=True, has_cuda=False, device='cuda:0'):
    ...
    def generate(self, graph: sp.lil_matrix):
        ...
        while True:
            ...
            xi_batch = xi_batch.to(self.device) if self.has_cuda else xi_batch
...
class DynGEMBatchPredictor:
    ...
    def get_predict_res(self, graph, model, batch_indices, counter, embedding_mat, x_pred):
        ...
        x_batch = x_batch.to(self.device) if self.has_cuda else x_batch
