...
class RegularizationLoss(nn.Module):
    ...
    def __init__(self, nu1, nu2, device):
    ...
    def forward(self, model):
        loss = Variable(torch.FloatTensor([0.]), requires_grad=True).to(self.device) if torch.cuda.is_available() else Variable(torch.FloatTensor([0.]), requires_grad=True)
...
class DynGraph2VecLoss(nn.Module):
    ...
    def __init__(self, beta, nu1, nu2, device):
        super(DynGraph2VecLoss, self).__init__()
        self.beta = beta
        self.regularization = RegularizationLoss(nu1, nu2, device)
...
class BatchGenerator:
    ...
    def __init__(self, node_list, batch_size, look_back, beta, shuffle=True, has_cuda=False, device='cuda:0'):
    ...
    def generate(self, graph_list):
        ...
        while True:
            ...
            x_pre_batch = x_pre_batch.to(self.device) if self.has_cuda else x_pre_batch
...
class BatchPredictor:
    ...
    def __init__(self, node_list, batch_size, has_cuda=False, device='cuda:0'):
        self.node_list = node_list
        self.node_num = len(node_list)
        self.batch_size = batch_size
        self.has_cuda = has_cuda
    ...
    def get_predict_res(self, graph_list, model, batch_indices, counter, look_back, embedding_mat, x_pred):
        ...
        x_pre_batches = x_pre_batches.to(self.device) if self.has_cuda else x_pre_batches
...
class DynamicEmbedding(BaseEmbedding):
    ...
    def __init__(self, base_path, origin_folder, embedding_folder, node_list, model, loss, batch_generator, batch_predictor, model_folder="model", has_cuda=False, device='cuda:0'):
        super(DynamicEmbedding, self).__init__(base_path, origin_folder, embedding_folder, node_list, model, loss, model_folder=model_folder, has_cuda=has_cuda, device=device)
...
def dyngem_embedding(method, args):
    ...
    data_loader = DataLoader(node_list, max_time_num, has_cuda=has_cuda, device=device)
...
#     for idx in range(end_idx - 1, end_idx):
    ...
    t2 = time.time()
    print('save to: {}'.format(base_path))
